...
ner_plot, = ax.plot(x_axis, np.zeros(100), 'r-', label=ACCEL_DOWN)
upp_plot, = ax.plot(x_axis, np.zeros(100), 'r-', label=ACCEL_UP)
EEPF_plot, = ax.plot(x_axis, np.zeros(100), 'g-', label='EEPF')
filtered_accel_sum_plot, = ax.plot(x_axis, np.zeros(100), 'b-', label='Filtered data')
...
while True:
    ...
    if been_down and been_up:
        ...
        if up_time[rep_count] < 0.5:
            extreme_explosive_power_factor[rep_count] = 1
        elif up_time[rep_count] > 10.5:
            extreme_explosive_power_factor[rep_count] = 0
        else:
            extreme_explosive_power_factor[rep_count] = (10.5 - up_time[rep_count]) * 0.1
    ...
    # Update the plot
    ner_plot.set_ydata(ACCEL_DOWN)
    upp_plot.set_ydata(ACCEL_UP)
    EEPF_plot.set_ydata(extreme_explosive_power_factor)
